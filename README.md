...
Windows QtWebEngine builds are handled separately by the private `Build QtWebEngine Windows` workflow, which produces
per-arch prefixes (`qtwebengine-windows-<ver>-x64` / `qtwebengine-windows-<ver>-arm64`) from the same repository.
The public Windows workflow (`.github/workflows/build-phraims-windows.yml`) downloads the matching QtWebEngine artifact
via `ci/get-qtwebengine-windows.ps1`, installs host Qt via `aqtinstall` when not provided, configures with Ninja after
seeding the VS2022 build environment via `vsdevcmd`, and uploads the per-arch build directory (`build/windows-<arch>`) as an artifact.
