...
each arch uses its runner, downloads the matching `qtwebengine-windows-<ver>-<arch>` artifact via `ci/get-qtwebengine-windows.ps1`,
installs host Qt via `aqtinstall` when not provided, configures with Ninja after seeding the VS2022 environment via `vsdevcmd`,
and builds `Phraims-Windows-<arch>` under `build/windows-<arch>`.
